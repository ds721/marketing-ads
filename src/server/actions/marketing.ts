...
function friendly(err: unknown, operation: string, tenantId?: string): string {
  ...
  if (err instanceof AiQuotaError) {
    ...
    if (err.provider === "Gemini" && err.reason === "rate_limited") {
      return "The free AI plan is used up for today, or Google is busy right now. Try again in a few minutes — nothing is wrong with your account. The free allowance resets daily.";
    }
